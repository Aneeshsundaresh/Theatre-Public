...
'''
''' use following to check for movie eligibility
cur.execute(" update Theater set Age=((select year(curdate())) - (select year(DOB) where Mobile_num=phonenum)) ")'''
...
db=sqlc.connect(host='localhost',user='root',passwd='Aneeshs2002',db='aneesh')
cur=db.cursor()
init_menu()
